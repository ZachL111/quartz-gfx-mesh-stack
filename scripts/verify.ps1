...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-quartz-gfx-mesh-detail.ps1

dart tests/test_domain_review.dart

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-quartz-gfx-mesh-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-quartz-gfx-mesh-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-quartz_gfx_mesh_stack-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
